 ...
 * 
 * PATCHED: Added independent OSM layer control (freeplay mode)
 * - Toggle lock/unlock to adjust map background independently
 * - River and sensors stay fixed while you pan/zoom the OSM tiles
 */
...
class RealRiverViewer {
    constructor(canvasId) {
        ...
        // ========== NEW: Independent OSM layer control ==========
        this.osmOffsetX = 0;        // Independent OSM pan X
        this.osmOffsetY = 0;        // Independent OSM pan Y
        this.osmZoomOffset = 3;    // OSM zoom level offset (-2 = more zoomed out)
        this.osmLocked = true;      // When false, drag/zoom affects OSM only
        // =========================================================
    ...
    calculateBounds() {
        // Use Klang Valley bounds to show entire metropolitan area (includes KL)
        this.bounds.lon_min = 101.3;
        this.bounds.lon_max = 101.8;
        this.bounds.lat_min = 2.9;
        this.bounds.lat_max = 3.3;

        // Set initial scale to fit entire Klang Valley
        this.fitToView();
    }
    ...
    // ========== NEW: OSM-specific coordinate transform ==========
    latLonToScreenOSM(lon, lat) {
        // Same as latLonToScreen but with OSM offsets applied
        const x = this.offsetX + this.osmOffsetX + lon * this.scale;
        const y = this.offsetY + this.osmOffsetY - lat * this.scale;
        return [x, y];
    }
    // =============================================================
    ...
    // ========== NEW: Toggle OSM lock ==========
    toggleOSMLock() {
        this.osmLocked = !this.osmLocked;
        this.updateLockButton();
        console.log(`OSM Layer: ${this.osmLocked ? 'LOCKED' : 'UNLOCKED - drag to adjust map'}`);
    }

    updateLockButton() {
        const btn = document.getElementById('toggleOSMLock');
        if (btn) {
            if (this.osmLocked) {
                btn.textContent = '🔒 Map Locked';
                btn.classList.remove('unlocked');
            } else {
                btn.textContent = '🔓 Adjusting Map...';
    ...
    resetOSMOffset() {
        this.osmOffsetX = 0;
        this.osmOffsetY = 0;
        this.osmZoomOffset = 0;
        console.log('OSM offset reset to 0');
    }
    // ============================================
    ...
    setupEventListeners() {
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            ...
            if (this.isDragging) {
                ...
                // ========== MODIFIED: Route to OSM or overlays ==========
                if (this.osmLocked) {
                    // Normal mode: move everything together
                    this.offsetX += dx;
                    this.offsetY += dy;
                } else {
                    // Freeplay mode: move only OSM background
                    this.osmOffsetX += dx;
                    this.osmOffsetY += dy;
                }
                // =========================================================
        ...
        this.canvas.addEventListener('wheel', (e) => {
            ...
            // ========== MODIFIED: Route zoom to OSM or overlays ==========
            if (this.osmLocked) {
                // Normal mode: zoom everything
                ...
                // Freeplay mode: adjust OSM zoom level offset
                if (e.deltaY > 0) {
                    this.osmZoomOffset = Math.max(-5, this.osmZoomOffset - 1);
                } else {
                    this.osmZoomOffset = Math.min(3, this.osmZoomOffset + 1);
                }
                console.log(`OSM Zoom Offset: ${this.osmZoomOffset}`);
            }
            // ==============================================================
        });
        ...
        document.getElementById('resetView')?.addEventListener('click', () => {
            this.fitToView();
            this.resetOSMOffset();  // Also reset OSM adjustments
        });
        ...
        // ========== NEW: OSM Lock toggle button ==========
        document.getElementById('toggleOSMLock')?.addEventListener('click', () => {
            this.toggleOSMLock();
        });

        // OSM zoom offset buttons
        document.getElementById('osmZoomOut')?.addEventListener('click', () => {
            this.osmZoomOffset = Math.max(-5, this.osmZoomOffset - 1);
            console.log(`OSM Zoom Offset: ${this.osmZoomOffset}`);
        });

        document.getElementById('osmZoomIn')?.addEventListener('click', () => {
            this.osmZoomOffset = Math.min(3, this.osmZoomOffset + 1);
            console.log(`OSM Zoom Offset: ${this.osmZoomOffset}`);
        });

        document.getElementById('resetOSM')?.addEventListener('click', () => {
            this.resetOSMOffset();
        });
        // ==================================================
    ...
    checkMarkerHover(mouseX, mouseY) {
        ...
        // ========== MODIFIED: Show different cursor when unlocked ==========
        if (this.osmLocked) {
            this.canvas.style.cursor = this.isDragging ? 'grabbing' : 'grab';
        } else {
            this.canvas.style.cursor = this.isDragging ? 'move' : 'crosshair';
        }
        // ====================================================================
    }
    ...
    render() {
        ...
        // ========== NEW: Show unlock indicator ==========
        if (!this.osmLocked) {
            this.drawUnlockIndicator();
        }
        // =================================================

        // Always scale up overlays (river and sensors) by 2x
        const savedScale = this.scale;
        const savedOffsetX = this.offsetX;
        const savedOffsetY = this.offsetY;

        // Find river center
        const riverCenterLon = 101.45;
        const riverCenterLat = 3.0;

        // Scale up from river center
        this.scale *= this.overlayScale;
        const [centerScreenX, centerScreenY] = [this.width / 2, this.height / 2];
        this.offsetX = centerScreenX - riverCenterLon * this.scale;
        this.offsetY = centerScreenY + riverCenterLat * this.scale;
    ...
    // ========== NEW: Visual indicator when unlocked ==========
    drawUnlockIndicator() {
        // Draw border to show adjustment mode
        ...
        this.ctx.fillRect(this.width / 2 - 150, 10, 300, 30);
        this.ctx.fillStyle = '#fff';
        this.ctx.font = 'bold 14px sans-serif';
        this.ctx.textAlign = 'center';
        this.ctx.fillText('🔓 ADJUSTING MAP - Drag to move, Scroll to zoom', this.width / 2, 30);

        // Show current offset values
        this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        this.ctx.fillRect(10, 50, 200, 60);
        this.ctx.fillStyle = '#fff';
        this.ctx.font = '12px monospace';
        this.ctx.textAlign = 'left';
        this.ctx.fillText(`OSM Offset X: ${this.osmOffsetX.toFixed(0)}px`, 20, 70);
        this.ctx.fillText(`OSM Offset Y: ${this.osmOffsetY.toFixed(0)}px`, 20, 85);
        this.ctx.fillText(`OSM Zoom: ${this.osmZoomOffset >= 0 ? '+' : ''}${this.osmZoomOffset}`, 20, 100);
    }
    // ==========================================================
    ...
    getOSMZoomLevel() {
        ...
        // ========== MODIFIED: Apply OSM zoom offset ==========
        return Math.max(8, Math.min(18, Math.floor(zoom) + this.osmZoomOffset));
        // ======================================================
    }
    ...
    drawOSMTiles() {
        ...
        // Use Klang Valley bounds for tiles
        const minLon = 101.3;
        const maxLon = 101.8;
        const minLat = 2.9;
        const maxLat = 3.3;
        ...
        for (let x = minTileX - 1; x <= maxTileX + 1; x++) {
            for (let y = minTileY - 1; y <= maxTileY + 1; y++) {
                ...
                // ========== MODIFIED: Use OSM-specific transform ==========
                const [tileX, tileY] = this.latLonToScreenOSM(tileLon, tileLat);
                // ===========================================================

                // Calculate tile width/height in screen pixels
                const [nextLon, nextLat] = this.tileToLatLon(x + 1, y + 1, zoom);
                const [nextX, nextY] = this.latLonToScreenOSM(nextLon, nextLat);
